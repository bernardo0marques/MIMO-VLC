...
for index = 1:length(modulationIndexVector) 
    ...
    for SNRIndex = 1:length(SNR)
        for j = 1:monteCarloLoops
            ...
            bitInputData = randi([0,1],(lengthbitInputData)/2,3); % porque +100??
            ...
            symbolDecoder = zeros (length(bitInputData)/2, 3);
            positionDecoder = zeros (length(bitInputData)/2, 3);
            
            bitInputDataSymbol = zeros (length(bitInputData), 3);
            bitInputDataPosition = zeros (length(bitInputData), 3);
            
            bitInputDataSymbol(1:2:end, 1:3) = bitInputData(1:2:end, 1:3);
            bitInputDataPosition(2:2:end, 1:3) = bitInputData(2:2:end, 1:3);
            
            symbolEncoder = bitInputData(1:2:end, 1:3);
            positionEncoder = bitInputData(2:2:end, 1:3);
                ...
                switch positionEncoder (k,1)
                    case 0
                        switch positionEncoder (k,2)
                            case 0
                                switch positionEncoder (k, 3)
                                    case 0
                                        encodedSymbol(k, :) = [pilot(k);0;0;0;0;0;0;0];
                                    case 1
                                        encodedSymbol(k, :) = [0;pilot(k);0;0;0;0;0;0];
                                end
                            case 1
                                switch positionEncoder (k, 3)
                                    case 1
                                        encodedSymbol(k, :) = [0;0;pilot(k);0;0;0;0;0];
                                    case 0
                                        encodedSymbol(k, :) = [0;0;0;pilot(k);0;0;0;0];
                                end
                        end
                    case 1
                        switch positionEncoder (k,2)
                            case 1
                                switch positionEncoder (k, 3)
                                    case 0
                                        encodedSymbol(k, :) = [0;0;0;0;pilot(k);0;0;0];
                                    case 1
                                        encodedSymbol(k, :) = [0;0;0;0;0;pilot(k);0;0];
                                end
                            case 0
                                switch positionEncoder (k, 3)
                                    case 1
                                        encodedSymbol(k, :) = [0;0;0;0;0;0;pilot(k);0];
                                    case 0
                                        encodedSymbol(k, :) = [0;0;0;0;0;0;0;pilot(k)];
                                end
                        end
                end
            end
...
%%
            ...
            for indChannel = 1:length(positionIdx)
                switch positionIdx (indChannel,1)
                    case 1 %000
                        decodedPosition(indChannel, :) = [0 0 0];
                    case 2 %001
                        decodedPosition(indChannel, :) = [0 0 1];
                    case 3 %011
                        decodedPosition(indChannel, :) = [0 1 1];
                    case 4 %010
                        decodedPosition(indChannel, :) = [0 1 0];
                    case 5 %100
                        decodedPosition(indChannel, :) = [1 1 0];
                    case 6 %101
                        decodedPosition(indChannel, :) = [1 1 1];
                    case 7 %111
                        decodedPosition(indChannel, :) = [1 0 1];
                    case 8 %110
                        decodedPosition(indChannel, :) = [1 0 0];
                end
            end
...
%%
            ...
            decDemodSymbolAux = pam8HardThreshold(symbolOutput);
...
%-------------------------Demodulation--------------------------
%%           
            bitOutputData = zeros (length(bitInputData), 3);
            bitOutputData(1:2:end) = [decodedSymbol(delay+1:bitInputDataLength,:) ;zeros(delay,3)];
            bitOutputData(2:2:end) = decodedPosition;
            
            bitOutputDataSymbol = zeros (length(bitInputData), 3);
            bitOutputDataPosition = zeros (length(bitInputData), 3);
            bitOutputDataSymbol(1:2:end) = [decodedSymbol(delay+1:bitInputDataLength,:) ;zeros(delay,3)];
