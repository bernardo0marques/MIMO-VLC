...
binaryInputDataMO = zeros((blockLength+INCREMENT)/2, 2, NumTx);
binaryOutputDataMO = zeros((blockLength+INCREMENT)/2, 2, NumTx);
%--------------------------------------------------------------------------
binaryInputData = randi([0,1],blockLength + INCREMENT,1); 
...
%%          
            ...
            if LS_EQUALIZATION == 1 % equalização COM sequência de treinamento
                for k = 1:NumRx
                    [f] = LSEqualizer(index, pilot, receivedVoltageSignal(:,k), numberOfBits, COD_GEOMETRY, LED_POSITION);  
                    output(:,k) = filter(f, 1, receivedVoltageSignal(:,k)) ;
                end
...
%%     
            for k = 1:NumRx                
                equalizedSignal(:,k) = pam4HardThreshold(output(:,k));
